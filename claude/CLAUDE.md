...
2. **Logo reveal** — ASCII art appears at screen center (`translateY(80px)` from its flex-group position), then animates to its final position (`translateY(0)`, `420ms cubic-bezier(0.2,0,0.2,1)`). ASCII font-size: `15px`. ASCII color: `--t-text` (beige, **not** `--t-accent` or `--t-surface`). Gap between ASCII and logo: `40px`. Logo size: `108×108px` of **actual visible content** (not the SVG canvas — source logos often have empty viewBox padding; crop it out via a CSS wrapper: render the SVG oversized so the content fills 108px, clip to 108×108, offset to center). Two-tone sine chime on entry. **These values are locked — source of truth is `/atelier/glaze/src/boot/`.**
